...
import sys
import math
from pytorch_quantization.nn import  TensorQuantizer, QuantLinear
...
ACT2FN = {"gelu": gelu, "bias_gelu": bias_gelu, "bias_tanh": bias_tanh, "relu": torch.nn.functional.relu, "swish": swish, "bias_noact": bias_noact}
...
class QuantizedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding = 0, dilation=1, groups=1, quant_desc_input=QuantLinear.default_quant_desc_input, quant_desc_weight=QuantLinear.default_quant_desc_weight, bias=True):
        ...
        self._aftergemm_quantizer = TensorQuantizer(quant_desc_input)
    ...
    def forward(self, input):
        ...
        output = self._aftergemm_quantizer(F.conv2d(input, weight, bias=None, stride=self.stride, padding=self.padding, dilation=self.dilation, groups=self.groups))
...
class LinearActivation(nn.Module):
    ...
    def __init__(self, in_features, out_features, quant_desc_input=QuantLinear.default_quant_desc_input, quant_desc_weight=QuantLinear.default_quant_desc_weight, act='gelu', bias=True, do_quant=True):
