...
class SCANNET_CTRS:
    ...
    def __init__(self, data_dir, split, depth_transform, label_type, margin, n_neg, is_embedding_set, dataset_threshold) -> None:
        print(f"==> Loading {split} data from {data_dir}, Depth transform: {depth_transform}, Label type: {label_type}, Margin: {margin}, N_neg: {n_neg}, Is_embedding_set: {is_embedding_set}, Dataset_threshold: {dataset_threshold:.3f}")
        ...
        assert split in ['train'], "Only support train split, use SCANNET for building test split"
        self.shared_transform = A.Compose([A.RandomResizedCrop(height=224, width=224, scale=(0.08, 1.0), ratio=(0.75, 1.3333), interpolation=cv2.INTER_CUBIC),
        ...
        for i in range(len(self.sample_stack['rgb'])):
            assert splitext(basename(self.sample_stack['rgb'][i]))[0] == splitext(basename(
                self.sample_stack['depth'][i]))[0], f"RGB and depth images not matched: {self.sample_stack['rgb'][i]}, {self.sample_stack['depth'][i]}"
...
if __name__ == '__main__':
    dataset_train = SCANNET_CTRS(split='train', data_dir='sunrgbd/RGB_JPG', depth_transform='rgb')
    dataset_test = SCANNET_CTRS(split='test', data_dir='sunrgbd/RGB_JPG', depth_transform='rgb')
