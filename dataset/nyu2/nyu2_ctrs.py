...
class NYU2_CTRS:
    ...
    def __init__(self, data_dir='nyu2', split='train', depth_transform='rgb', label_type='clip_vitb32', margin=0.1, n_neg=3, is_embedding_set=False, dataset_threshold=0.99) -> None:
        self.sample_stack = {}
        self.margin = margin
        self.n_neg = n_neg
        self.split = split
        ...
        print(f"==> margin: {self.margin}, n_neg: {self.n_neg}, dataset_threshold: {dataset_threshold}")
        ...
        assert split in ['train'], "Only support train split, use SUNRGBD for building test dataset"
        ...
        if self.label_type == 'clip_vitb32':
            ...
            for file in self.sample_stack['rgb']:
                self.clip_labels.append(int(self.clip_predictions_df.loc[basename(file)]['class_id']))
                self.similarities.append(self.clip_predictions_df.loc[basename(file)]['similarity'])
        ...
        if self.is_embedding_set == False:
            self.embedding_cache = None # ([5000, 512]) # how much memory does it take? 5000 * 512 * 4 = 10MB
...
if __name__ == '__main__':
    dataset_train = NYU2_CTRS(split='train', data_dir='../../dbs/nyu2', depth_transform='rgb')
    dataset_train_embedding = NYU2_CTRS(split='train', data_dir='../../dbs/nyu2', depth_transform='rgb', is_embedding_set=True)
    print(f"Train: {len(dataset_train)},  Train Embedding: {len(dataset_train_embedding)}")
